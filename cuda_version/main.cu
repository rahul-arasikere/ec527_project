...
void call_kernel(img_ptr_t gpu_memory, img_ptr_t cpu_memory, int *count, int width, int height,
                 void (*kernel)(img_ptr_t, int *, const int, const int),
...
int main(int argc, char **argv)
{
    ...
    CUDA_SAFE_CALL(cudaMalloc((img_t**)&gpu_memory, width * height * sizeof(img_t)));
    CUDA_SAFE_CALL(cudaMalloc((img_t**)&image, width * height * sizeof(img_t)));
    CUDA_SAFE_CALL(cudaMemcpy(gpu_memory, input, width * height * sizeof(img_t), cudaMemcpyHostToDevice));
    CUDA_SAFE_CALL(cudaBindTexture(0, image, gpu_memory, width * height * sizeof(img_t)));
    ...
    CUDA_SAFE_CALL(cudaPeekAtLastError());
    call_kernel(gpu_memory, cpu_border, count, width, height, border_kernel, 
                blocksPerGrid, threadsPerBlock);
    call_kernel(gpu_memory, cpu_minima, count, width, height, minima_basin_kernel, 
                blocksPerGrid, threadsPerBlock);
    call_kernel(gpu_memory, cpu_watershed, count, width, height, watershed_kernel, 
                blocksPerGrid, threadsPerBlock);
...
void call_kernel(img_ptr_t gpu_memory, img_ptr_t cpu_memory, int *count, int width, int height,
    ...
    *count = 0;
    ...
    {
        _old = _new;
        kernel<<<blocksPerGrid, threadsPerBlock>>>(gpu_memory, count, width, height);
    ...
    CUDA_SAFE_CALL(cudaMemcpy(cpu_memory, gpu_memory, width * height * sizeof(img_t), cudaMemcpyDeviceToHost));
}
